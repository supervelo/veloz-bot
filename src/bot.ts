...
export class swapBot {
    ...
    listenCommand() {
        ...
        this.bot.onText(/\/bindChannel/, async (msg, match) => {
            if (msg.from.username == this.params.adminName) {
                ...
                this.bot.sendMessage(msg.chat.id, `Bind push channel successfully`)
            } else {
                this.bot.sendMessage(msg.chat.id, `You are not an administrator and cannot set it up`)
            }
        });
    ...
    async changeChainPrc(msg: Message) {
        ...
        user.log_id = 0
        this.updateUserSetting(user)
        this.bot.sendMessage(msg.chat.id, "Set prc successfully")
    }
    //设置跟单配置
            ...
            if (wallets.length) {
                ...
                if (find) {
                    ...
                    if (result) {
                        ...
                        user.log_id = 0
                        this.updateUserSetting(user)
                        this.bot.sendMessage(msg.chat.id, "Deletion of bound wallet address successful")
                    } else {
                        this.bot.sendMessage(msg.chat.id, "Failed to delete bound wallet address")
                    }
                } else {
                    this.bot.sendMessage(msg.chat.id, "This wallet address is not bound")
                }
            } else {
                this.bot.sendMessage(msg.chat.id, "No wallet has been bound yet")
            }
        } else {
            this.bot.sendMessage(msg.chat.id, "Incorrect wallet address")
        }
    }
    //生成钱包
    async generateWallet(query) {
        ...
        if (wallet.length + 1 > 5) {
            this.bot.sendMessage(query.message.chat.id, `Exceeding binding limit`)
        } else {
            let insertItem = {
            ...
            if (result) {
                ...
                this.bot.sendMessage(query.message.chat.id, `Failed to bind account`)
            }
        }
    }
    //添加抢开盘
    async addRush(msg: Message) {
        ...
        if (find) {
            ...
            if (findTask) {
                ...
                user.log_id = 0
                this.updateUserSetting(user)
                this.bot.sendMessage(msg.chat.id, `There is already an opening task for this contract`)
                return
            }
            let insert = {
    ...
    async addWallet(msg: Message) {
        ...
        try {
            ...
            if (wallet.length + 1 > 5) {
                this.bot.sendMessage(msg.chat.id, `Exceeding binding limit`)
            } else {
                let find = await this.db.find("wallet", [`telegram_id=${msg.from.id}`, `address='${account.address}'`])
                if (find) {
                    this.bot.sendMessage(msg.chat.id, `Bind account repeatedly`)
                } else {
                    let insertItem = {
                        address: account.address,
                    ...
                    if (result) {
                        ...
                        user.log_id = 0
                        this.updateUserSetting(user)
                        this.bot.sendMessage(msg.chat.id, `Account binding successful`)
                    } else {
                        this.bot.sendMessage(msg.chat.id, `Failed to bind account`)
                    }
                }
            }
        } catch (error) {
    ...
    async addWatch(msg: Message) {
        ...
        if (hasErrAddress) {
            this.bot.sendMessage(msg.chat.id, "There is a wrong address")
        } else {
            let watchList = await this.db.select("watch", [`telegram_id=${msg.from.id}`])
            if (watchList.length + addressArr.length > 20) {
                this.bot.sendMessage(msg.chat.id, "Binding limit exceeded")
            } else {
                let insertList = []
                addressArr.forEach(item => {
    ...
    async rushDetail(query: any, id: Number) {
        ...
        if (contract) {
            ...
            this.bot.sendMessage(query.message.chat.id, "Contract information not found")
        }
    }
    ...
    async switchRouter(query: any) {
        switch (query.data) {
            ...
            //删除钱包
            case "delete_wallet":
                this.bot.sendMessage(query.message.chat.id, `Please enter wallet address to delete`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
            ...
            case "smart_money":
                if (query.from.username == this.params.adminName) {
                    ...
                    this.bot.sendMessage(query.message.chat.id, `You are not an administrator and cannot set it up`)
                }
                break
            case "set_prc":
                if (query.from.username == this.params.adminName) {
            ...
            case "Ethereum":
                this.bot.sendMessage(query.message.chat.id, 
                    `Please enter the Ethereum prc link to replace`,
                    {
                        "reply_markup": {
                            "force_reply": true
                        }
                ...
                break
            case "Arbitrum":
                this.bot.sendMessage(query.message.chat.id, `Please enter Arbitrum prc link replacement`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
                ...
                break
            case "Goerli":
                this.bot.sendMessage(query.message.chat.id, `Please enter the Goerli prc link to replace`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
                ...
                break
            case "Solana":
                this.bot.sendMessage(query.message.chat.id, `Please enter the Solana prc link to replace`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
            ...
            //导入钱包
            case "import_wallet":
                this.bot.sendMessage(query.message.chat.id, `Please enter the private key to bind`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
            ...
            case "add_watch":
                this.bot.sendMessage(query.message.chat.id, 
                    `Please enter the addresses that need to be monitored. If there are multiple addresses, please use separate addresses such as 0x, 0x.`
                    , {
                        "reply_markup": {
                            "force_reply": true
                        }
        ...
        if (pickerTaskWallet) {
            ...
            if (task) {
                ...
                this.bot.sendMessage(query.message.chat.id, `Query task failed`)
                return
            }
        }
        ...
        if (followBuyMatch) {
            ...
            if (find) {
                if (!find.follow_private_key) {
                    this.bot.sendMessage(query.message.chat.id, `Please select wallet first`)
                    return
                }
        ...
        if (followGasMatch) {
            ...
            this.bot.sendMessage(query.message.chat.id, `Please enter Gas Tip`
                , {
                    "reply_markup": {
                        "force_reply": true
                    }
        ...
        if (followSwapFeeMatch) {
            ...
            this.bot.sendMessage(query.message.chat.id, `Please enter Slippage`
                , {
                    "reply_markup": {
                        "force_reply": true
                    }
        ...
        if (followBuyAmountMatch) {
            ...
            this.bot.sendMessage(query.message.chat.id, `Please enter the purchase amount in ETH`
                , {
                    "reply_markup": {
                        "force_reply": true
                    }
        ...
        if (taskGasMatch) {
            ...
            this.bot.sendMessage(query.message.chat.id, `Please enter gas`
                , {
                    "reply_markup": {
                        "force_reply": true
                    }
        ...
        if (bindWatchNameMatch) {
            this.bot.sendMessage(query.message.chat.id, 
                `👇👇👇 Please remark the name for the following address. Please enter the name that needs to be remarked.\n${bindWatchNameMatch[1]}`
                , {
                    "reply_markup": {
                        "force_reply": true
                    }
    ...
    async handleManualSell(msg: any, chainId: number, address: string, percent: number, swapFee: number, gasFee: number, type: number, logId?: number) {
        ...
        if (logId) {
            ...
            if (wallet) {
                ...
                this.bot.sendMessage(msg.message.chat.id, 'The wallet of this order has been deleted')
                return
            }
        } else {
            let user = await this.getUserSetting(msg.from.id)
        ...
        if (this.chainIds.indexOf(chainId) == -1) {
            this.bot.sendMessage(msg.message.chat.id, 'The chain id is invalid')
            return
        }
        if (!web3.utils.isAddress(address)) {
        ...
        if (typeof percent != 'number') {
            this.bot.sendMessage(msg.message.chat.id, 'Wrong selling ratio')
            return
        }
        let contract = await this.db.find("contract", [`address='${address}'`])
        if (!contract || !contract.address) {
        ...
        if (fastPrice && fastPrice.price) {
            ...
            errorTamplate(this.bot, msg, contract, Number((Number(amountIn) / (10 ** Number(contract.decimals))).toFixed(3)), "", type, "Failed to obtain currency price")
        }
    }
    //卖出
    ...
    async handleManualBuy(msg: any, chainId: number, address: string, amount: number, swapFee: number, gasFee: number, type: number, logId?: number) {
        ...
        if (typeof amount != 'number') {
            this.bot.sendMessage(msg.message.chat.id, 'Wrong amount')
            return
        }
        let contract = await this.db.find("contract", [`address='${address}'`])
        if (!contract || !contract.address) {
        ...
        if (fastPrice && fastPrice.price) {
            ...
            errorTamplate(this.bot, msg, contract, amount, "", type, "Failed to obtain currency price")
        }
    }
    //跟单买入
    ...
    async querySmarkMoney(msg: Message) {
        ...
        if (hasError) {
            ...
            if (list.length) {
                this.bot.sendMessage(msg.chat.id, "The contract has been entered repeatedly")
            } else {
                this.bot.sendMessage(msg.chat.id, "Start looking for smart money")
                ...
                this.bot.sendMessage(msg.chat.id, "Smart money has entered")
            }
        }
    }
    //返回首页
